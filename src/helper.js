...
export const getRandomValue = async(length) => {
  const msg = await jscu.random.getRandomBytes(length);
  return await jscu.hash.compute(msg, 'SHA-256');
};

export const createAsset = async(userId, idsLength=IDsLength) => {
...
export const createAssetWithoutFile = async (userId, idsLength=IDsLength) => {
  const bbcAsset = new BBcAsset(userId, idsLength);
  await bbcAsset.setRandomNonce();
  const assetBody = new Uint8Array(32);
  for (let i = 0; i < 32; i++) {
...
export const convertNumberToBinary = (number) => {
  return (number >>> 0);
};
