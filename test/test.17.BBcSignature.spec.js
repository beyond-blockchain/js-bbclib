...
describe(`${envName}: Test BBcSignature`, () => {
  ...
  it('serialize and deserialize', async () => {
    ...
    expect(bbcSignature.key_type).to.equal(signature_unpack.key_type);
  ...
  it('serialize and deserialize with KeyType == 0', async () => {
    ...
    const bbcSignature = new bbclib.BBcSignature(para.KeyType.NOT_INITIALIZED);
    const signature_pack = bbcSignature.pack();
    const signature_unpack = new bbclib.BBcSignature(para.KeyType.NOT_INITIALIZED);
    await signature_unpack.unpack(signature_pack);
