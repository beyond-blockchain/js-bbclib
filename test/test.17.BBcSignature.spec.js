...
describe(`${envName}: Test BBcSignature`, () => {
  ...
  it('serialize and deserialize', async () => {
    ...
    const signature = new BBcSignature(para.KeyType.ECDSA_P256v1);
    ...
    const signatureUnpack = new BBcSignature(para.KeyType.ECDSA_P256v1);
  ...
  it('serialize and deserialize with KeyType == 0', async () => {
    const signature = new BBcSignature(para.KeyType.NOT_INITIALIZED);
    const signatureBin = signature.pack();
    const signatureUnpack = new BBcSignature(para.KeyType.NOT_INITIALIZED);
    await signatureUnpack.unpack(signatureBin);
    expect(signature.keyType).to.equal(signatureUnpack.keyType);
  });
